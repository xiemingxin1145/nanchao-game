...
// V25.0 楼船谍影版：水军海战 / 谍报纵横 所需常量与判定（只读引用，不改 navy.js / espionage.js）
import { NAVY_UNITS, NAVY_UNIT_KEYS, RIVER_CITIES, isWaterLink, isNavalBattle, canBuildNavy, getAvailableNavyTypes } from './navy.js';
...
export class UI {
  ...
  initGameUI() {
      ...
      <div class="game-screen">
        ...
        <div class="top-bar">
          ...
          <div class="top-bar-btns">
            ...
            <button class="btn-small v25-navy-btn" id="btn-v25-navy" title="V25.0 水军：沿江沿海战船 / 楼船蒙冲突击 / 水寨水师">水军</button>
            <button class="btn-small v26-aris-btn" id="btn-v26-aris" title="V26.0 门阀：士族谱牒 / 清谈玄辩 / 联姻世好 / 寒门拔擢">门阀</button>
            <button class="btn-small v27-rel-btn" id="btn-v27-rel" title="V27.0 佛道：斋醮法会 / 僧官制度 / 灭佛崇佛 / 石窟开凿">佛道</button>
            <button class="btn-small v28-fin-btn" id="btn-v28-fin" title="V28.0 财政：盐铁专卖 / 铸钱通货 / 均输平准 / 岁入岁出">财政</button>
    ...
    // V25.0：楼船谍影 — 水军面板入口
    const bv25n = document.getElementById('btn-v25-navy'); if (bv25n) bv25n.onclick = () => this.showNavyPanelV25();
    // V26.0：门阀清谈 — 士族谱牒面板入口
    const bv26a = document.getElementById('btn-v26-aris'); if (bv26a) bv26a.onclick = () => this.showAristocratPanelV26();
    // V27.0：玄门佛国 — 佛道深化面板入口
    const bv27r = document.getElementById('btn-v27-rel'); if (bv27r) bv27r.onclick = () => this.showReligionPanelV27();
    // V28.0：盐铁通货 — 财政面板入口
    const bv28f = document.getElementById('btn-v28-fin'); if (bv28f) bv28f.onclick = () => this.showFinancePanelV28();
    ...
    panel.innerHTML = `
      ...
      ${isMine ? `
        <div class="action-buttons">
          ...
          ${(typeof RIVER_CITIES !== 'undefined' && RIVER_CITIES.includes(city.id)) ? `
          <button class="btn-small v25-navy-btn" onclick="__ui_.showWaterFortPanelV25('${city.id}')" title="V25.0 水寨：沿江建寨 / 造船训卒 / 控扼江河">⚓ 水寨</button>
          ` : ''}
    ...
    const rows = otherFactions.map(f => {
      ...
      return `<div class="v15-dip-row" style="--v15-fac:${f.color};border-left-color:${f.color}">
        ...
        <div class="v15-dip-actions">
          ...
          <button class="btn-small v25-vert-btn" onclick="__ui_.showVerticalPanelV25('${f.id}')" title="V25.0 纵横术：离间君臣 / 散布流言 / 策反骁将 / 反间破谋">🪤 纵横</button>
        </div>
      </div>`;
    }).join('');
  ...
  showFarmPanelV24() {
    ...
    const rows = myCities.map(c => {
      ...
      const cityYield = this._v24FarmYieldOf(c);
      ...
      return `<div class="v24-farm-row">
        <div class="v24-farm-info">
          ...
          <span class="v24-farm-attr">屯田 Lv.${lv} · 年产粮 <i class="v24-yield">🌾 ${cityYield}</i> · 征调劳力 ${labor} 人</span>
  ...
  // ============================================================
  // V25.0「楼船谍影版」— 水军海战 / 谍报纵横深化
  //  约定：新类名一律 v25- 前缀；所有 game/navy/espionage 调用
  //        带 typeof 守卫，优雅降级；战船/水寨/纵横为表现层派生，
  //        轻量状态挂 game._v25，不改动 navy.js / espionage.js 等数据模块。
  // ============================================================

  // ---------- V25 通用工具 ----------
  _v25Num(v, d = 0) { return (typeof v === 'number' && isFinite(v)) ? v : d; }
  _v25Esc(s) {
    return String(s == null ? '' : s).replace(/[&<>"]/g, c =>
      ({ '&': '&amp;', '<': '&lt;', '>': '&lt;', '"': '&quot;' }[c]));
  }
  // V25 轻量表现层状态（战船数 / 水寨等级 / 纵横成果）
  _v25Store() {
    const g = this.game;
    if (!g) return { fleet: {}, waterFort: {}, vert: {} };
    if (!g._v25) g._v25 = { fleet: {}, waterFort: {}, vert: {} };
    return g._v25;
  }
  // V25 古风弹窗骨架（复用 v24 视觉语系，独立 v25- 前缀）
  _v25ModalShell(titleHtml, bodyHtml, extraCls = '') {
    const modal = document.createElement('div');
    modal.className = 'modal-overlay v25-overlay';
    modal.innerHTML = `
      <div class="modal v25-modal v25-scroll ${extraCls}">
        <div class="v25-corner tl"></div><div class="v25-corner tr"></div>
        <div class="v25-corner bl"></div><div class="v25-corner br"></div>
        <h2 class="modal-title v25-title">${titleHtml}</h2>
        <div class="v25-body">${bodyHtml}</div>
        <button class="btn-ancient v25-close" onclick="this.closest('.v25-overlay').remove()">关闭</button>
      </div>`;
    document.body.appendChild(modal);
    if (this.audio && typeof this.audio.playPanelOpen === 'function') {
      try { this.audio.playPanelOpen(); } catch (e) {}
  ...
  // V25 通知横幅
  _v25Notify(title, text, type) {
    const cls = type === 'success' ? 'v15-notify-success' : type === 'warn' ? 'v15-notify-warn' : '';
    const bar = document.createElement('div');
    bar.className = `v15-notify v25-notify ${cls}`;
    bar.innerHTML = `<b>${this._v25Esc(title)}</b><span>${this._v25Esc(text || '')}</span>`;
    document.body.appendChild(bar);
    requestAnimationFrame(() => bar.classList.add('show'));
    setTimeout(() => { bar.classList.remove('show'); setTimeout(() => bar.remove(), 600); }, 3200);
  }
  ...
  // ============================================================
  // 一、水军面板（各沿江沿海城市战船数量 / 类型 + 海战地图可视化）
  // ============================================================
  _v25FleetOf(city) {
    const st = this._v25Store();
    if (!st.fleet[city.id]) st.fleet[city.id] = { louchuan: 0, mengchong: 0, douchian: 0 };
    return st.fleet[city.id];
  }
  _v25IsRiverCity(city) {
    if (typeof RIVER_CITIES === 'undefined' || !RIVER_CITIES) return false;
    return RIVER_CITIES.includes(city.id);
  }
  showNavyPanelV25() {
    if (!this.game) { this.toast('开始游戏后可见水军'); return; }
    const g = this.game;
    const me = g.playerFaction;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : {};
    // 我方沿江沿海城市
    let myCities = [];
    try { g.cities.forEach(c => { if (c.owner === me && this._v25IsRiverCity(c)) myCities.push(c); }); } catch (e) {}
    // 全部沿江城市（用于海战地图）
    let allRiver = [];
    try { allRiver = (typeof RIVER_CITIES !== 'undefined' && RIVER_CITIES) ? RIVER_CITIES.map(id => {
      const c = (typeof g.getCity === 'function') ? g.getCity(id) : (g.cities && g.cities.get(id));
      return c || null;
    }).filter(Boolean) : []; } catch (e) { allRiver = []; }

    // 战船类型元数据
    const units = (typeof NAVY_UNITS !== 'undefined') ? NAVY_UNITS : {};
    const unitKeys = (typeof NAVY_UNIT_KEYS !== 'undefined' && NAVY_UNIT_KEYS.length) ? NAVY_UNIT_KEYS : ['louchuan', 'mengchong', 'douchian'];

    const rows = myCities.map(c => {
      const fl = this._v25FleetOf(c);
      const total = this._v25Num(fl.louchuan) + this._v25Num(fl.mengchong) + this._v25Num(fl.douchian);
      const canBuild = (typeof canBuildNavy === 'function') ? canBuildNavy(c) : this._v25IsRiverCity(c);
      const wfLv = this._v25WaterFortLevel(c);
      return `<div class="v25-fleet-row">
        <div class="v25-fleet-info">
          <b>${this._v25Esc(c.name)} <small class="v25-fleet-own">${c.owner === me ? '我方' : '敌'}</small></b>
          <span class="v25-fleet-attr">水寨 Lv.${wfLv} · 战船 <i class="v25-fleet-total">${total}</i> 艘</span>
          <div class="v25-fleet-bar"><div class="v25-fleet-fill" style="width:${Math.min(100, total * 4)}%"></div></div>
        </div>
        <div class="v25-fleet-types">
          <span class="v25-unit-chip v25-unit-lc" title="楼船：主力大船，水战+50%">🛡 楼船 ${this._v25Num(fl.louchuan)}</span>
          <span class="v25-unit-chip v25-unit-mc" title="蒙冲：快速突击，水战+60%">⚡ 蒙冲 ${this._v25Num(fl.mengchong)}</span>
          <span class="v25-unit-chip v25-unit-dc" title="斗舰：攻防均衡">⚔ 斗舰 ${this._v25Num(fl.douchian)}</span>
        </div>
        ${canBuild
          ? `<button class="btn-small v25-build-btn" onclick="__ui_._v25BuildShip('${c.id}')">造船<small>金 ${80 + total * 15}</small></button>`
          : '<span class="v25-fleet-no">非港</span>'}
      </div>`;
    }).join('');

    // 海战地图可视化：横向河流航道节点
    const totalShips = myCities.reduce((s, c) => {
      const fl = this._v25FleetOf(c);
      return s + this._v25Num(fl.louchuan) + this._v25Num(fl.mengchong) + this._v25Num(fl.douchian);
    }, 0);
    const nodes = allRiver.map((c, i) => {
      const mine = c.owner === me;
      const fl = this._v25FleetOf(c);
      const t = this._v25Num(fl.louchuan) + this._v25Num(fl.mengchong) + this._v25Num(fl.douchian);
      return `<div class="v27-map-node ${mine ? 'mine' : ''}">
        <div class="v27-map-dot"></div>
        <div class="v27-map-name">${this._v25Esc(c.name)}</div>
        ${mine && t > 0 ? `<div class="v27-map-ship">🚢 ${t}</div>` : ''}
      </div>`;
    }).join('<div class="v27-map-link"></div>');

    const body = `
      <div class="v25-summary">
        <div class="v25-stat"><span>沿江港城</span><b>${myCities.length} 座</b></div>
        <div class="v25-stat"><span>战船总数</span><b style="color:var(--v25-wave)">🚢 ${totalShips}</b></div>
        <div class="v25-stat"><span>楼船主力</span><b style="color:var(--v25-louchuan)">${myCities.reduce((s, c) => s + this._v25Num(this._v25FleetOf(c).louchuan), 0)}</b></div>
        <div class="v25-stat"><span>府库金</span><b>${this._v25Num(res.money)}</b></div>
      </div>
      <div class="v25-status-line v25-wait">楼船下益州，金陵王气黯然收。沿江造船、水寨为基，控扼江河则南北可图。</div>
      <div class="v25-section"><div class="v25-sec-title">◆ 长江航道图（水军巡弋）</div>
        <div class="v27-river-map">${nodes}</div>
      </div>
      <div class="v25-section"><div class="v25-sec-title">◆ 各港战船编练</div>
        <div class="v25-fleet-list">${rows || '<p class="v25-empty">我方尚无沿江港城，可于建康/江陵/江州等地营建水寨。</p>'}</div>
      </div>
      <div class="v25-section"><div class="v25-sec-title">◆ 战船制式</div>
        <div class="v25-unit-docs">
          ${unitKeys.map(k => {
            const u = units[k] || {};
            return `<div class="v25-unit-doc">
              <b>${this._v25Esc(u.name || k)}</b>
              <span>水战加成 ${Math.round((this._v25Num(u.waterBonus, 1) - 1) * 100)}% · 造价 ${this._v25Num(u.cost)}金/人</span>
              <small>${this._v25Esc(u.desc || '')}</small>
            </div>`;
          }).join('')}
        </div>
      </div>`;
    this._v25ModalShell('🚢 楼船横江 · 水战纵横', body, 'v25-wide');
  }
  _v25BuildShip(cityId) {
    const g = this.game; if (!g) return;
    const city = (typeof g.getCity === 'function') ? g.getCity(cityId) : (g.cities && g.cities.get(cityId));
    if (!city || city.owner !== g.playerFaction) { this.toast('非我之城，不可造船'); return; }
    if (!this._v25IsRiverCity(city)) { this.toast('此城不通江河，不可造船'); return; }
    const fl = this._v25FleetOf(city);
    const total = this._v25Num(fl.louchuan) + this._v25Num(fl.mengchong) + this._v25Num(fl.douchian);
    const cost = 80 + total * 15;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res) { this.toast('资源系统未就绪'); return; }
    if (this._v25Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    res.money -= cost;
    // 按水寨等级决定可造舰型：Lv0 斗舰，Lv3+ 蒙冲，Lv6+ 楼船
    const wfLv = this._v25WaterFortLevel(city);
    let pick;
    if (wfLv >= 6) pick = Math.random() < 0.4 ? 'louchuan' : (Math.random() < 0.5 ? 'mengchong' : 'douchian');
    else if (wfLv >= 3) pick = Math.random() < 0.5 ? 'mengchong' : 'douchian';
    else pick = 'douchian';
    fl[pick] = this._v25Num(fl[pick]) + 1;
    const un = (typeof NAVY_UNITS !== 'undefined' && NAVY_UNITS[pick]) ? NAVY_UNITS[pick].name : pick;
    this.toast(`${city.name} 新造 ${un} 一艘（耗 ${cost} 金）`);
    if (this.audio && this.audio.playCoin) { try { this.audio.playCoin(); } catch (e) {} }
    document.querySelectorAll('.v25-overlay').forEach(m => m.remove());
    this.showNavyPanelV25();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  ...
  // 二、水寨建造（沿江城市）
  // ============================================================
  _v25WaterFortLevel(city) {
    const st = this._v25Store();
    return Math.max(0, Math.min(10, st.waterFort[city.id] != null ? st.waterFort[city.id] : 0));
  }
  showWaterFortPanelV25(cityId) {
    if (!this.game) { this.toast('开始游戏后可建水寨'); return; }
    const g = this.game;
    const city = (typeof g.getCity === 'function') ? g.getCity(cityId) : (g.cities && g.cities.get(cityId));
    if (!city) { this.toast('城池不存在'); return; }
    if (city.owner !== g.playerFaction) { this.toast('非我之城，不可建寨'); return; }
    if (!this._v25IsRiverCity(city)) { this.toast('此城不在江河沿线，不可建水寨'); return; }
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : {};
    const lv = this._v25WaterFortLevel(city);
    const fl = this._v25FleetOf(city);
    const total = this._v25Num(fl.louchuan) + this._v25Num(fl.mengchong) + this._v25Num(fl.douchian);
    const costGold = 100 + lv * 50, costWood = 150 + lv * 80;
    const maxed = lv >= 10;
    const fortNames = ['芦苇泊', '木栅水寨', '浮桥港', '楼船坞', '临江巨寨'];
    const fn = lv === 0 ? '无寨' : fortNames[Math.min(fortNames.length - 1, Math.floor((lv - 1) / 2))];
    const body = `
      <div class="v25-summary">
        <div class="v25-stat"><span>港城</span><b>${this._v25Esc(city.name)}</b></div>
        <div class="v25-stat"><span>水寨等级</span><b style="color:var(--v25-wave)">Lv.${lv}</b></div>
        <div class="v25-stat"><span>驻泊战船</span><b>🚢 ${total}</b></div>
        <div class="v25-stat"><span>水战加成</span><b>+${lv * 12}%</b></div>
      </div>
      <div class="v25-fort-stage">
        <div class="v25-fort-icon">⚓</div>
        <div class="v25-fort-name">${this._v25Esc(fn)}</div>
        <div class="v25-fort-bar"><div class="v25-fort-fill" style="width:${lv * 10}%"></div></div>
      </div>
      <div class="v25-status-line v25-wait">水寨为水军之基：每级水寨 +12% 水战攻防，Lv3 可造蒙冲，Lv6 可造楼船。</div>
      ${maxed
        ? '<div class="v25-status-line v25-ok">此港已为临江巨寨，千帆竞发，长江天堑尽为我用。</div>'
        : `<button class="btn-ancient v25-fort-up" onclick="__ui_._v25UpgradeWaterFort('${city.id}')">⚓ 营建水寨（${costGold}金 / ${costWood}粮）</button>`}
      <div class="v25-section"><div class="v25-sec-title">◆ 水寨沿革</div>
        <p class="v25-fort-desc">自芦苇泊而木栅水寨，自浮桥港而楼船巨坞。寨成则水军有依，楼船可泊，长江天堑反为我之长堑。</p>
      </div>`;
    this._v25ModalShell('⚓ 临江水寨 · 楼船坞', body, 'v25-wide');
  }
  _v25UpgradeWaterFort(cityId) {
    const g = this.game; if (!g) return;
    const city = (typeof g.getCity === 'function') ? g.getCity(cityId) : (g.cities && g.cities.get(cityId));
    if (!city || city.owner !== g.playerFaction) { this.toast('无权建寨'); return; }
    const st = this._v25Store();
    const lv = this._v25WaterFortLevel(city);
    if (lv >= 10) { this.toast('水寨已至极品'); return; }
    const costGold = 100 + lv * 50, costWood = 150 + lv * 80;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res) { this.toast('资源系统未就绪'); return; }
    if (this._v25Num(res.money) < costGold) { this.toast(`金钱不足（需 ${costGold} 金）`); return; }
    if (this._v25Num(res.food) < costWood) { this.toast(`粮草不足（需 ${costWood} 粮）`); return; }
    res.money -= costGold; res.food -= costWood;
    st.waterFort[city.id] = lv + 1;
    this.toast(`${city.name} 水寨升至 Lv.${lv + 1}`);
    if (this.audio && this.audio.playCoin) { try { this.audio.playCoin(); } catch (e) {} }
    document.querySelectorAll('.v25-overlay').forEach(m => m.remove());
    this.showWaterFortPanelV25(cityId);
    if (typeof this.showCityPanel === 'function' && this.game.selectedCity === cityId) this.showCityPanel(city);
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }

  // ============================================================
  // 三、谍报深化：渗透 / 反间 / 离间 / 刺杀（V25 扩展面板）
  // ============================================================
  showEspionagePanelV25() {
    if (!this.game) { this.toast('开始游戏后可行纵横'); return; }
    const g = this.game;
    const me = g.playerFaction;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : { money: 0 };
    // 候选密探
    let spies = [];
    ...
    const otherFactions = Object.values(FACTIONS).filter(f => f.id !== me);
    const V25_OPS = [
      { id: 'infiltrate', name: '渗透', cost: 800, desc: '潜入住邸：长期潜伏，每回合降低该势力 5 点治安', cap: 0.72 },
      { id: 'counter',    name: '反间', cost: 600, desc: '反间破谋：识破并驱逐敌国在我境内的细作', cap: 0.68 },
      { id: 'alienate',   name: '离间', cost: 1200, desc: '离间君臣：散布流言，降低其君主与谋臣信任', cap: 0.58 },
      { id: 'assassinate', name: '刺杀', cost: 2000, desc: '刺客枭首：刺杀其麾下名将（风险极高）', cap: 0.42 }
    ];
    const spyOpts = spies.length ? spies.map(s =>
      `<option value="${s.id}">${this._v25Esc(s.name)}（智${this._v25Num(s.effIntel || s.intel, 50)}）</option>`).join('') : '<option value="">无可用密探</option>';
    const facOpts = otherFactions.map(f =>
      `<option value="${f.id}">${this._v25Esc(f.name)}</option>`).join('');
    const opOpts = V25_OPS.map(o =>
      `<option value="${o.id}">${o.name}（耗${o.cost}金）</option>`).join('');
    // 历史记录（表现层）
    const st = this._v25Store();
    const hist = Array.isArray(st.vert.log) ? st.vert.log : (st.vert.log = []);
    const histRows = hist.slice(-10).reverse().map(h =>
      `<div class="v25-spy-row v25-spy-${h.ok ? 'ok' : 'fail'}"><b>${this._v25Esc(h.op)}</b>
        <span>→ ${this._v25Esc(h.fac)}</span><span class="v25-spy-result">${h.ok ? '✓ 得手' : '✗ 败露'}</span></div>`).join('');

    const body = `
      <div class="v25-summary">
        <div class="v25-stat"><span>可用密探</span><b>${spies.length} 人</b></div>
        <div class="v25-stat"><span>府库金</span><b>${this._v25Num(res.money)}</b></div>
        <div class="v25-stat"><span>已遣纵横</span><b>${hist.length} 次</b></div>
        <div class="v25-stat"><span>得手率</span><b style="color:var(--v25-wave)">${hist.length ? Math.round(hist.filter(h => h.ok).length / hist.length * 100) + '%' : '—'}</b></div>
      </div>
      <div class="v25-status-line v25-wait">纵横捭阖：上兵伐谋，其次伐交。密探智越高，行事越易成；然事败则身死名裂，慎之。</div>
      <div class="v25-spy-form">
        <label class="v25-field"><span>密探武将</span><select id="v25-spy-gen">${spyOpts}</select></label>
        <label class="v25-field"><span>目标势力</span><select id="v25-spy-fac">${facOpts}</select></label>
        <label class="v25-field"><span>纵横行动</span><select id="v25-spy-op">${opOpts}</select></label>
        <div class="v25-spy-prob"><span>预估成功率</span><b id="v25-spy-prob-num">—</b></div>
        <button class="btn-ancient" id="v25-spy-send">遣出死间</button>
      </div>
      <div class="v25-spy-mdesc" id="v25-spy-mdesc">${V25_OPS[0].desc}</div>
      <h3 class="v25-sub">纵横录</h3>
      <div class="v25-spy-list">${histRows || '<p class="v25-empty">尚无纵横行动记录。</p>'}</div>`;
    const modal = this._v25ModalShell('🗡 死间纵横 · 谍影重重', body, 'v25-wide');
    // 联动预估成功率
    const genSel = modal.querySelector('#v25-spy-gen');
    const opSel = modal.querySelector('#v25-spy-op');
    const mDesc = modal.querySelector('#v25-spy-mdesc');
    const probNum = modal.querySelector('#v25-spy-prob-num');
    const refresh = () => {
      const gen = spies.find(s => s.id === (genSel && genSel.value));
      const intel = gen ? this._v25Num(gen.effIntel || gen.intel, 50) : 50;
      const op = V25_OPS.find(o => o.id === (opSel && opSel.value)) || V25_OPS[0];
      let p = Math.min(op.cap, intel / 160);
      p = Math.round(p * 100);
      if (probNum) { probNum.textContent = p + '%'; probNum.style.color = p > 60 ? '#6fcf6f' : p > 40 ? '#e8a33c' : '#e05555'; }
      if (mDesc) mDesc.textContent = op.desc;
    };
    if (opSel) opSel.onchange = refresh;
    if (genSel) genSel.onchange = refresh;
    refresh();
    const sendBtn = modal.querySelector('#v25-spy-send');
    if (sendBtn) sendBtn.onclick = () => this._v25DoEspionage(
      genSel ? genSel.value : '',
      modal.querySelector('#v25-spy-fac') ? modal.querySelector('#v25-spy-fac').value : '',
      opSel ? opSel.value : 'infiltrate');
  }
  _v25DoEspionage(generalId, targetFid, opId) {
    const g = this.game; if (!g) return;
    const gen = (typeof g.getGeneral === 'function') ? g.getGeneral(generalId) : null;
    if (!gen) { this.toast('请选择密探武将'); return; }
    const opMap = { infiltrate: { cost: 800, name: '渗透' }, counter: { cost: 600, name: '反间' },
      alienate: { cost: 1200, name: '离间' }, assassinate: { cost: 2000, name: '刺杀' } };
    const op = opMap[opId] || opMap.infiltrate;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v25Num(res.money) < op.cost) { this.toast(`金钱不足（需 ${op.cost} 金）`); return; }
    res.money -= op.cost;
    // 成功率：智力/160，按行动上限裁剪
    const caps = { infiltrate: 0.72, counter: 0.68, alienate: 0.58, assassinate: 0.42 };
    const intel = this._v25Num(gen.effIntel || gen.intel, 50);
    const chance = Math.min(caps[opId] || 0.6, intel / 160);
    const ok = Math.random() < chance;
    const st = this._v25Store();
    if (!Array.isArray(st.vert.log)) st.vert.log = [];
    const facName = (FACTIONS[targetFid] && FACTIONS[targetFid].name) || targetFid;
    st.vert.log.push({ op: op.name, fac: facName, ok, turn: g.turn });
    let msg;
    if (ok) {
      // 落地效果：调用 espionageAction 若可用，否则仅表现层
      try {
        if (typeof espionageAction === 'function') {
          gen.game = g; gen.faction = g.playerFaction;
          const actMap = { infiltrate: 'gather_info', counter: 'gather_info', alienate: 'spread_rumors', assassinate: 'defector' };
          espionageAction(gen, targetFid, actMap[opId] || 'gather_info');
        }
      } catch (e) {}
      msg = `${gen.name}【${op.name}】${facName} 得手，敌营震动。`;
      this._v25Notify('纵横', msg, 'success');
    } else {
      msg = `${gen.name}【${op.name}】事泄败露，仅以身免。`;
      this._v25Notify('纵横', msg, 'warn');
    }
    if (this.audio && this.audio.playCoin) { try { this.audio.playCoin(); } catch (e) {} }
    document.querySelectorAll('.v25-overlay').forEach(m => m.remove());
    if (this.game.state === 'playing') this.showEspionagePanelV25();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }

  // ============================================================
  // 四、纵横术面板（外交界面入口：策反 / 离间 / 流言）
  // ============================================================
  showVerticalPanelV25(targetFid) {
    if (!this.game) { this.toast('开始游戏后可行纵横'); return; }
    const g = this.game;
    const me = g.playerFaction;
    const f = FACTIONS[targetFid];
    if (!f) { this.toast('势力不存在'); return; }
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : { money: 0 };
    // 可策反敌将（低忠诚）
    let bribable = [];
    try { bribable = g.getFactionGenerals(targetFid).filter(x => x.loyalty < 50 && x.role !== '君主')
      .sort((a, b) => (a.loyalty || 0) - (b.loyalty || 0)).slice(0, 5); } catch (e) {}
    // 候选说客
    let speakers = [];
    try { speakers = g.getFactionGenerals(me).filter(x => !x.inArmy && !x.onHostage && x.role !== '君主')
      .sort((a, b) => (b.effIntel || b.intel || 0) - (a.effIntel || a.intel || 0)).slice(0, 6); } catch (e) {}

    const bribRows = bribable.length ? bribable.map(b => {
      const lo = this._v25Num(b.loyalty, 50);
      const prob = Math.max(10, Math.min(85, Math.round((60 - lo) * 1.5)));
      return `<div class="v25-vb-row">
        <b>${this._v25Esc(b.name)}</b>
        <span class="v25-vb-loy">忠诚 ${lo}</span>
        <span class="v25-vb-prob">${prob}%</span>
        <button class="btn-small v25-vb-do" onclick="__ui_._v25BribeGeneral('${targetFid}','${b.id}')">重金策反</button>
      </div>`;
    }).join('') : '<p class="v25-empty">其营中暂无低忠可诱之将。</p>';

    const speakerOpts = speakers.length ? speakers.map(s =>
      `<option value="${s.id}">${this._v25Esc(s.name)}（智${this._v25Num(s.effIntel || s.intel, 50)}）</option>`).join('') : '<option value="">无可遣说客</option>';

    const body = `
      <div class="v25-summary">
        <div class="v25-stat"><span>目标</span><b style="color:${f.color}">${this._v25Esc(f.name)}</b></div>
        <div class="v25-stat"><span>可诱之将</span><b>${bribable.length} 人</b></div>
        <div class="v25-stat"><span>可用说客</span><b>${speakers.length} 人</b></div>
        <div class="v25-stat"><span>府库金</span><b>${this._v25Num(res.money)}</b></div>
      </div>
      <div class="v25-status-line v25-wait">纵横之道：以利啖之，以疑间之。敌将忠诚越低，策反越易成；流言一起，君臣自疑。</div>
      <div class="v25-section"><div class="v25-sec-title">◆ 说客遣派（流言 / 离间）</div>
        <div class="v25-vb-form">
          <label class="v25-field"><span>遣谁</span><select id="v25-vb-speaker">${speakerOpts}</select></label>
          <button class="btn-small" onclick="__ui_._v25SpreadRumor('${targetFid}')">📢 散布流言（800金）</button>
          <button class="btn-small" onclick="__ui_._v25Alienate('${targetFid}')">🪤 离间君臣（1500金）</button>
        </div>
      </div>
      <div class="v25-section"><div class="v25-sec-title">◆ 策反敌将</div>
        <div class="v25-vb-list">${bribRows}</div>
      </div>`;
    this._v25ModalShell(`🪤 纵横策反 · ${this._v25Esc(f.name)}`, body, 'v25-wide');
  }
  _v25BribeGeneral(targetFid, generalId) {
    const g = this.game; if (!g) return;
    const cost = 1000;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v25Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    let result = { ok: false, msg: '策反接口未就绪' };
    try {
      if (g.diplomacy && typeof g.diplomacy.bribeGeneral === 'function') {
        result = g.diplomacy.bribeGeneral(g, targetFid, generalId, cost);
      } else {
        // 表现层兜底：直接降忠诚
        const gen = (typeof g.getGeneral === 'function') ? g.getGeneral(generalId) : null;
        if (gen) { gen.loyalty = Math.max(0, this._v25Num(gen.loyalty, 50) - 30); result = { ok: true, msg: `${gen.name} 收受重金，心渐归我。` }; }
      }
    } catch (e) { result = { ok: false, msg: String(e.message || e) }; }
    this._v25Notify('策反', result.msg, result.ok ? 'success' : 'warn');
    if (result.ok) res.money -= cost;
    document.querySelectorAll('.v25-overlay').forEach(m => m.remove());
    if (this.game.state === 'playing') this.showVerticalPanelV25(targetFid);
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  _v25SpreadRumor(targetFid) {
    const g = this.game; if (!g) return;
    const cost = 800;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v25Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    ...
    let n = 0;
    try {
      const gens = g.getFactionGenerals(targetFid) || [];
      gens.forEach(x => { if (x.role !== '君主') { x.loyalty = Math.max(0, this._v25Num(x.loyalty, 50) - (3 + Math.floor(Math.random() * 5))); n++; } });
    } catch (e) {}
    this._v25Notify('流言', `流言传入 ${(FACTIONS[targetFid] && FACTIONS[targetFid].name) || targetFid}，${n} 名武将忠诚下降。`, 'success');
    document.querySelectorAll('.v25-overlay').forEach(m => m.remove());
    if (this.game.state === 'playing') this.showVerticalPanelV25(targetFid);
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  _v25Alienate(targetFid) {
    const g = this.game; if (!g) return;
    const cost = 1500;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v25Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    ...
    try {
      const gens = g.getFactionGenerals(targetFid) || [];
      gens.sort((a, b) => this._v25Num(b.effIntel || b.intel) - this._v25Num(a.effIntel || a.intel));
      gens.slice(0, 3).forEach(x => { x.loyalty = Math.max(0, this._v25Num(x.loyalty, 50) - (10 + Math.floor(Math.random() * 8))); n++; });
    } catch (e) {}
    this._v25Notify('离间', `离间计成，${(FACTIONS[targetFid] && FACTIONS[targetFid].name) || targetFid} 君臣相疑，${n} 名重臣离心。`, 'success');
    document.querySelectorAll('.v25-overlay').forEach(m => m.remove());
    if (this.game.state === 'playing') this.showVerticalPanelV25(targetFid);
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  ...
  // ============================================================
  // V26.0「门阀清谈版」— 士族谱牒 / 清谈玄辩 / 联姻世好 / 寒门拔擢
  //  约定：新类名一律 v26- 前缀；所有 game/family 调用带 typeof 守卫；
  //        族望/清谈次数为表现层派生，轻量状态挂 game._v26，不改数据模块。
  // ============================================================
  _v26Num(v, d = 0) { return (typeof v === 'number' && isFinite(v)) ? v : d; }
  _v26Esc(s) {
    return String(s == null ? '' : s).replace(/[&<>"]/g, c =>
      ({ '&': '&amp;', '<': '&lt;', '>': '&lt;', '"': '&quot;' }[c]));
  }
  _v26Store() {
    const g = this.game;
    if (!g) return { qingtan: 0, hanmen: 0, log: [] };
    if (!g._v26) g._v26 = { qingtan: 0, hanmen: 0, log: [] };
    return g._v26;
  }
  _v26ModalShell(titleHtml, bodyHtml, extraCls = '') {
    const modal = document.createElement('div');
    modal.className = 'modal-overlay v26-overlay';
    modal.innerHTML = `
      <div class="modal v26-modal v26-scroll ${extraCls}">
        <div class="v26-corner tl"></div><div class="v26-corner tr"></div>
        <div class="v26-corner bl"></div><div class="v26-corner br"></div>
        <h2 class="modal-title v26-title">${titleHtml}</h2>
        <div class="v26-body">${bodyHtml}</div>
        <button class="btn-ancient v26-close" onclick="this.closest('.v26-overlay').remove()">关闭</button>
      </div>`;
    document.body.appendChild(modal);
    if (this.audio && typeof this.audio.playPanelOpen === 'function') {
      try { this.audio.playPanelOpen(); } catch (e) {}
  ...
  _v26Notify(title, text, type) {
    const cls = type === 'success' ? 'v15-notify-success' : type === 'warn' ? 'v15-notify-warn' : '';
    const bar = document.createElement('div');
    bar.className = `v15-notify v26-notify ${cls}`;
    bar.innerHTML = `<b>${this._v26Esc(title)}</b><span>${this._v26Esc(text || '')}</span>`;
    document.body.appendChild(bar);
    requestAnimationFrame(() => bar.classList.add('show'));
    setTimeout(() => { bar.classList.remove('show'); setTimeout(() => bar.remove(), 600); }, 3200);
  }
  // 族望派生：按武将综合才名 + 现有官职品级估算
  _v26ClanRank(gen) {
    const talent = this._v26Num(gen.politics) + this._v26Num(gen.intel) + this._v26Num(gen.command) + this._v26Num(gen.force);
    // 才名 0~400 → 族望 0~100
    const fame = Math.round(Math.min(100, talent / 4));
    // 高门姓氏加成（历史大族：王谢袁萧、崔卢李郑）
    const highNames = ['王', '谢', '袁', '萧', '崔', '卢', '李', '郑', '裴', '杨', '韦', '杜'];
    const surname = String(gen.name || '').charAt(0);
    const clanBoost = highNames.includes(surname) ? 15 : 0;
    return Math.min(100, fame + clanBoost);
  }
  _v26GradeOf(rank) {
    if (rank >= 75) return { name: '上品 · 高门', cls: 'v26-gt-top' };
    if (rank >= 45) return { name: '中品 · 次门', cls: 'v26-gt-mid' };
    return { name: '下品 · 寒门', cls: 'v26-gt-low' };
  }
  showAristocratPanelV26() {
    if (!this.game) { this.toast('开始游戏后可见门阀'); return; }
    const g = this.game;
    const me = g.playerFaction;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : {};
    const st = this._v26Store();
    // 我方在任/闲居武将
    let gens = [];
    try { gens = g.getFactionGenerals(me).filter(x => x.role !== '君主').slice().sort((a, b) => this._v26ClanRank(b) - this._v26ClanRank(a)); } catch (e) {}
    const topRows = gens.slice(0, 12).map(x => {
      const rk = this._v26ClanRank(x);
      const gt = this._v26GradeOf(rk);
      const talent = this._v26Num(x.politics) + this._v26Num(x.intel) + this._v26Num(x.command) + this._v26Num(x.force);
      return `<div class="v26-clan-row">
        <div class="v26-clan-info">
          <b>${this._v26Esc(x.name)}</b>
          <span class="v26-clan-attr">才名 ${talent} · ${gt.name}</span>
          <div class="v26-clan-bar"><div class="v26-clan-fill ${gt.cls}" style="width:${rk}%"></div></div>
        </div>
        <span class="v26-clan-rank">${rk}</span>
      </div>`;
    }).join('');
    // 寒门可拔擢（族望<45 但才名≥200）
    const hanmen = gens.filter(x => this._v26ClanRank(x) < 45 && (this._v26Num(x.politics) + this._v26Num(x.intel) + this._v26Num(x.command) + this._v26Num(x.force)) >= 200).slice(0, 5);
    const hanRows = hanmen.map(x =>
      `<div class="v26-han-row">
        <b>${this._v26Esc(x.name)}</b>
        <span class="v26-han-talent">才名 ${this._v26Num(x.politics) + this._v26Num(x.intel) + this._v26Num(x.command) + this._v26Num(x.force)}</span>
        <button class="btn-small v26-han-btn" onclick="__ui_._v26Promote('${x.id}')">拔擢</button>
      </div>`).join('');
    // 清谈记录
    const logRows = (st.log || []).slice(-6).reverse().map(h =>
      `<div class="v26-log-row"><b>${this._v26Esc(h.what)}</b><span>${this._v26Esc(h.desc)}</span></div>`).join('');

    const body = `
      <div class="v26-summary">
        <div class="v26-stat"><span>在朝士族</span><b>${gens.length} 家</b></div>
        <div class="v26-stat"><span>上品高门</span><b style="color:var(--v26-gold)">${gens.filter(x => this._v26ClanRank(x) >= 75).length}</b></div>
        <div class="v26-stat"><span>清谈次数</span><b style="color:var(--v26-purple)">${st.qingtan || 0}</b></div>
        <div class="v26-stat"><span>府库金</span><b>${this._v26Num(res.money)}</b></div>
      </div>
      <div class="v26-status-line v26-wait">上品无寒门，下品无势族。清谈玄辩以扬名，联姻世好以固位；然寒门有才，亦当拔擢以广贤路。</div>
      <div class="v26-section"><div class="v26-sec-title">◆ 士族谱牒 · 才名望族</div>
        <div class="v26-clan-list">${topRows || '<p class="v26-empty">麾下无人。</p>'}</div>
      </div>
      <div class="v26-section"><div class="v26-sec-title">◆ 清谈玄辩（每月一度）</div>
        <button class="btn-ancient v26-qt-btn" onclick="__ui_._v26Qingtan()" title="聚名士清谈老庄，提升文化声望，吸引人才">🎙 开坛清谈（500金）</button>
      </div>
      <div class="v26-section"><div class="v26-sec-title">◆ 联姻世好</div>
        <div class="v26-marriage-tip">欲与高门世好联姻，请赴「家族」面板遣女出嫁、纳妇联姻。</div>
      </div>
      <div class="v26-section"><div class="v26-sec-title">◆ 寒门拔擢（破门阀之见）</div>
        <div class="v26-han-list">${hanRows || '<p class="v26-empty">麾下暂无可用寒门才士。</p>'}</div>
      </div>
      <div class="v26-section"><div class="v26-sec-title">◆ 清谈雅集录</div>
        <div class="v26-log-list">${logRows || '<p class="v26-empty">尚未开坛。</p>'}</div>
      </div>`;
    this._v26ModalShell('🏮 门阀清谈 · 谱牒品藻', body, 'v26-wide');
  }
  _v26Qingtan() {
    const g = this.game; if (!g) return;
    const cost = 500;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v26Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    res.money -= cost;
    const st = this._v26Store();
    st.qingtan = this._v26Num(st.qingtan) + 1;
    // 表现层效果：文化+少量，随机吸引在野人才提示
    const topics = ['老庄玄理', '人物品藻', '周易玄远', '竹林遗风', '声无哀乐'];
    const topic = topics[Math.floor(Math.random() * topics.length)];
    if (!Array.isArray(st.log)) st.log = [];
    st.log.push({ what: `清谈·${topic}`, desc: `聚名士于竹林，三玄并辩，士林传为佳话。` });
    // 若文化系统存在，+文化值
    try { if (typeof g.addCulture === 'function') g.addCulture(20 + Math.floor(Math.random() * 15)); } catch (e) {}
    this._v26Notify('清谈', `开坛讲「${topic}」，文化声望+，士林归附者众。`, 'success');
    if (this.audio && this.audio.playCoin) { try { this.audio.playCoin(); } catch (e) {} }
    document.querySelectorAll('.v26-overlay').forEach(m => m.remove());
    this.showAristocratPanelV26();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  _v26Promote(genId) {
    const g = this.game; if (!g) return;
    ...
    if (!gen) { this.toast('武将不存在'); return; }
    const st = this._v26Store();
    st.hanmen = this._v26Num(st.hanmen) + 1;
    // 表现层：提拔后该武将忠诚度/能力小幅提升
    if (typeof gen.loyalty === 'number') gen.loyalty = Math.min(100, gen.loyalty + 15);
    if (typeof gen.politics === 'number') gen.politics = Math.min(100, gen.politics + 2);
    if (!Array.isArray(st.log)) st.log = [];
    st.log.push({ what: `拔擢·${gen.name}`, desc: `不问出身，破格提拔，寒门之士感激涕零。` });
    this._v26Notify('拔擢', `破格拔擢 ${gen.name}，朝野为之震动，寒门归心。`, 'success');
    document.querySelectorAll('.v26-overlay').forEach(m => m.remove());
    this.showAristocratPanelV26();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  ...
  // ============================================================
  // V27.0「玄门佛国版」— 斋醮法会 / 僧官制度 / 灭佛崇佛 / 石窟开凿
  //  约定：新类名一律 v27- 前缀；所有 game/religion 调用带 typeof 守卫；
  //        法会/僧官/国策为表现层派生，轻量状态挂 game._v27，不改数据模块。
  // ============================================================
  _v27Num(v, d = 0) { return (typeof v === 'number' && isFinite(v)) ? v : d; }
  _v27Esc(s) {
    return String(s == null ? '' : s).replace(/[&<>"]/g, c =>
      ({ '&': '&amp;', '<': '&lt;', '>': '&lt;', '"': '&quot;' }[c]));
  }
  _v27Store() {
    const g = this.game;
    if (!g) return { festival: 0, policy: 'neutral', monks: {}, log: [] };
    if (!g._v27) g._v27 = { festival: 0, policy: 'neutral', monks: {}, log: [] };
    return g._v27;
  }
  _v27ModalShell(titleHtml, bodyHtml, extraCls = '') {
    const modal = document.createElement('div');
    modal.className = 'modal-overlay v27-overlay';
    modal.innerHTML = `
      <div class="modal v27-modal v27-scroll ${extraCls}">
        <div class="v27-corner tl"></div><div class="v27-corner tr"></div>
        <div class="v27-corner bl"></div><div class="v27-corner br"></div>
        <h2 class="modal-title v27-title">${titleHtml}</h2>
        <div class="v27-body">${bodyHtml}</div>
        <button class="btn-ancient v27-close" onclick="this.closest('.v27-overlay').remove()">关闭</button>
      </div>`;
    document.body.appendChild(modal);
    if (this.audio && typeof this.audio.playPanelOpen === 'function') {
      try { this.audio.playPanelOpen(); } catch (e) {}
  ...
  _v27Notify(title, text, type) {
    const cls = type === 'success' ? 'v15-notify-success' : type === 'warn' ? 'v15-notify-warn' : '';
    const bar = document.createElement('div');
    bar.className = `v15-notify v27-notify ${cls}`;
    bar.innerHTML = `<b>${this._v27Esc(title)}</b><span>${this._v27Esc(text || '')}</span>`;
    document.body.appendChild(bar);
    requestAnimationFrame(() => bar.classList.add('show'));
    setTimeout(() => { bar.classList.remove('show'); setTimeout(() => bar.remove(), 600); }, 3200);
  }
  showReligionPanelV27() {
    if (!this.game) { this.toast('开始游戏后可见佛道'); return; }
    const g = this.game;
    const me = g.playerFaction;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : {};
    const st = this._v27Store();
    let cities = [];
    try { cities = g.getFactionCities(me) || []; } catch (e) { cities = []; }
    // 统计
    let bud = 0, dao = 0, conf = 0, grotto = 0, totalCulture = 0;
    cities.forEach(c => {
      const b = (c.buildings && c.buildings.buddhist_temple) || (c.religion && c.religion.buddhist) || 0;
      const d = (c.buildings && c.buildings.daoist_temple) || (c.religion && c.religion.daoist) || 0;
      const cf = (c.buildings && c.buildings.confucian_temple) || (c.religion && c.religion.confucian) || 0;
      const gt = (c.buildings && c.buildings.grotto) || 0;
      bud += b; dao += d; conf += cf; grotto += gt;
      totalCulture += (c.religion && c.religion.culture) || 0;
    });
    const polName = { neutral: '中道平衡', favor: '崇佛奉道', suppress: '沙汰沙门' }[st.policy] || '中道';
    const polCls = st.policy === 'favor' ? 'v27-pol-favor' : st.policy === 'suppress' ? 'v27-pol-suppress' : 'v27-pol-neutral';
    // 石窟候选城
    let grottoCities = [];
    try { grottoCities = (typeof GROTTO_CITIES !== 'undefined' && GROTTO_CITIES) ? GROTTO_CITIES.map(id => {
      const c = (typeof g.getCity === 'function') ? g.getCity(id) : (g.cities && g.cities.get(id));
      return c && c.owner === me ? c : null;
    }).filter(Boolean) : []; } catch (e) {}
    const grotRows = grottoCities.map(c => {
      const gLv = (c.buildings && c.buildings.grotto) || 0;
      return `<div class="v27-grot-row">
        <b>${this._v27Esc(c.name)}</b>
        <span class="v27-grot-lv">石窟 Lv.${gLv}/3</span>
        ${gLv >= 3 ? '<span class="v27-grot-max">已臻化境</span>'
          : `<button class="btn-small v27-grot-btn" onclick="__ui_._v27BuildGrotto('${c.id}')">开凿<small>1000金 500粮</small></button>`}
      </div>`;
    }).join('');
    // 法会记录
    const logRows = (st.log || []).slice(-6).reverse().map(h =>
      `<div class="v27-log-row"><b>${this._v27Esc(h.what)}</b><span>${this._v27Esc(h.desc)}</span></div>`).join('');

    const body = `
      <div class="v27-summary">
        <div class="v27-stat"><span>佛寺</span><b style="color:var(--v27-gold)">🛕 ${bud}</b></div>
        <div class="v27-stat"><span>道观</span><b style="color:var(--v27-jade)">☯ ${dao}</b></div>
        <div class="v27-stat"><span>孔庙</span><b style="color:var(--v27-ink)">📖 ${conf}</b></div>
        <div class="v27-stat"><span>石窟</span><b style="color:var(--v27-grotto)">⛰ ${grotto}</b></div>
      </div>
      <div class="v27-summary">
        <div class="v27-stat"><span>总文化值</span><b>${Math.round(totalCulture)}</b></div>
        <div class="v27-stat"><span>法会次数</span><b>${st.festival || 0}</b></div>
        <div class="v27-stat"><span>国教政策</span><b class="${polCls}">${polName}</b></div>
        <div class="v27-stat"><span>府库金</span><b>${this._v27Num(res.money)}</b></div>
      </div>
      <div class="v27-status-line v27-wait">南朝四百八十寺，多少楼台烟雨中。佛道盛衰，系于国政；法会斋醮，以祈福祉。</div>
      <div class="v27-section"><div class="v27-sec-title">◆ 斋醮法会（每月一度）</div>
        <div class="v27-fest-btns">
          <button class="btn-ancient v27-fest-budd" onclick="__ui_._v27Festival('buddhist')">🛕 佛事水陆法会（600金）</button>
          <button class="btn-ancient v27-fest-dao" onclick="__ui_._v27Festival('daoist')">☯ 道门罗天大醮（600金）</button>
        </div>
      </div>
      <div class="v27-section"><div class="v27-sec-title">◆ 国教政策（国策切换）</div>
        <div class="v27-pol-btns">
          <button class="btn-small ${st.policy==='favor'?'on':''}" onclick="__ui_._v27SetPolicy('favor')">崇佛奉道</button>
          <button class="btn-small ${st.policy==='neutral'?'on':''}" onclick="__ui_._v27SetPolicy('neutral')">中道平衡</button>
          <button class="btn-small ${st.policy==='suppress'?'on':''}" onclick="__ui_._v27SetPolicy('suppress')">沙汰沙门</button>
        </div>
        <p class="v27-pol-desc">${st.policy === 'favor' ? '崇佛奉道：民心+、文化+，但赋税-。' : st.policy === 'suppress' ? '沙汰沙门：灭佛僧还俗，金粮+，但民心大降、文化停滞。' : '中道平衡：不偏不倚，佛道并尊。'}</p>
      </div>
      <div class="v27-section"><div class="v27-sec-title">◆ 石窟开凿（仅平城/洛阳/建康）</div>
        <div class="v27-grot-list">${grotRows || '<p class="v27-empty">我方未据石窟名城。</p>'}</div>
      </div>
      <div class="v27-section"><div class="v27-sec-title">◆ 斋醮录</div>
        <div class="v27-log-list">${logRows || '<p class="v27-empty">尚未举办法会。</p>'}</div>
      </div>`;
    this._v27ModalShell('☸ 玄门佛国 · 梵音道韵', body, 'v27-wide');
  }
  _v27Festival(religion) {
    const g = this.game; if (!g) return;
    const cost = 600;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v27Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    res.money -= cost;
    const st = this._v27Store();
    st.festival = this._v27Num(st.festival) + 1;
    const isBud = religion === 'buddhist';
    const events = isBud
      ? ['水陆道场超度孤魂', '讲经法会缁林云集', '佛像开光万众瞻仰']
      : ['罗天大醮禳灾祈福', '符箓斋醮炼丹授箓', '天师升坛说经演道'];
    const ev = events[Math.floor(Math.random() * events.length)];
    if (!Array.isArray(st.log)) st.log = [];
    st.log.push({ what: isBud ? '佛事' : '道斋', desc: ev });
    try { if (typeof g.addCulture === 'function') g.addCulture(15 + Math.floor(Math.random() * 10)); } catch (e) {}
    this._v27Notify(isBud ? '水陆法会' : '罗天大醮', `${ev}，民心归附，文化增益。`, 'success');
    if (this.audio && this.audio.playCoin) { try { this.audio.playCoin(); } catch (e) {} }
    document.querySelectorAll('.v27-overlay').forEach(m => m.remove());
    this.showReligionPanelV27();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  _v27SetPolicy(pol) {
    const g = this.game; if (!g) return;
    const st = this._v27Store();
    st.policy = (pol === 'favor' || pol === 'suppress') ? pol : 'neutral';
    const names = { favor: '崇佛奉道', neutral: '中道平衡', suppress: '沙汰沙门' };
    this._v27Notify('国策', `国教政策改为：${names[st.policy]}。`, st.policy === 'suppress' ? 'warn' : 'success');
    document.querySelectorAll('.v27-overlay').forEach(m => m.remove());
    this.showReligionPanelV27();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  _v27BuildGrotto(cityId) {
    const g = this.game; if (!g) return;
    const city = (typeof g.getCity === 'function') ? g.getCity(cityId) : (g.cities && g.cities.get(cityId));
    if (!city || city.owner !== g.playerFaction) { this.toast('非我之城'); return; }
    // 调用现有石窟建造接口
    let result = { ok: false, msg: '石窟开凿接口未就绪' };
    try {
      if (typeof canBuildGrotto === 'function' && !canBuildGrotto(city)) { result = { ok: false, msg: '此城不可开凿石窟' }; }
      else if (g.cityBuild && typeof g.cityBuild === 'function') result = g.cityBuild(cityId, 'grotto');
    } catch (e) { result = { ok: false, msg: String(e.message || e) }; }
    this._v27Notify('石窟', result.msg, result.ok ? 'success' : 'warn');
  ...
  // ============================================================
  // V28.0「盐铁通货版」— 盐铁专卖 / 铸钱通货 / 均输平准 / 岁入岁出
  //  约定：新类名一律 v28- 前缀；所有 game/tax 调用带 typeof 守卫；
  //        专卖等级/铸钱量为表现层派生，轻量状态挂 game._v28，不改数据模块。
  // ============================================================
  _v28Num(v, d = 0) { return (typeof v === 'number' && isFinite(v)) ? v : d; }
  _v28Esc(s) {
    return String(s == null ? '' : s).replace(/[&<>"]/g, c =>
      ({ '&': '&amp;', '<': '&lt;', '>': '&lt;', '"': '&quot;' }[c]));
  }
  _v28Store() {
    const g = this.game;
    if (!g) return { salt: 0, iron: 0, coin: 0, level: 1 };
    if (!g._v28) g._v28 = { salt: 0, iron: 0, coin: 0, level: 1 };
    return g._v28;
  }
  _v28ModalShell(titleHtml, bodyHtml, extraCls = '') {
    const modal = document.createElement('div');
    modal.className = 'modal-overlay v28-overlay';
    modal.innerHTML = `
      <div class="modal v28-modal v28-scroll ${extraCls}">
        <div class="v28-corner tl"></div><div class="v28-corner tr"></div>
        <div class="v28-corner bl"></div><div class="v28-corner br"></div>
        <h2 class="modal-title v28-title">${titleHtml}</h2>
        <div class="v28-body">${bodyHtml}</div>
        <button class="btn-ancient v28-close" onclick="this.closest('.v28-overlay').remove()">关闭</button>
      </div>`;
    document.body.appendChild(modal);
    if (this.audio && typeof this.audio.playPanelOpen === 'function') {
      try { this.audio.playPanelOpen(); } catch (e) {}
  ...
  _v28Notify(title, text, type) {
    const cls = type === 'success' ? 'v15-notify-success' : type === 'warn' ? 'v15-notify-warn' : '';
    const bar = document.createElement('div');
    bar.className = `v15-notify v28-notify ${cls}`;
    bar.innerHTML = `<b>${this._v28Esc(title)}</b><span>${this._v28Esc(text || '')}</span>`;
    document.body.appendChild(bar);
    requestAnimationFrame(() => bar.classList.add('show'));
    setTimeout(() => { bar.classList.remove('show'); setTimeout(() => bar.remove(), 600); }, 3200);
  }
  showFinancePanelV28() {
    if (!this.game) { this.toast('开始游戏后可见财政'); return; }
    const g = this.game;
    const me = g.playerFaction;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : {};
    const st = this._v28Store();
    let cities = [];
    try { cities = g.getFactionCities(me) || []; } catch (e) { cities = []; }
    const n = cities.length || 1;
    // 岁入：基础商税 + 盐铁专卖 + 铸钱
    const baseTax = Math.round(n * 120);
    const saltRev = st.salt * 200;
    const ironRev = st.iron * 180;
    const coinRev = st.coin * 80;
    const totalIn = baseTax + saltRev + ironRev + coinRev;
    // 通胀率：铸钱量/城池数 → 越高物价越贵
    const inflation = Math.min(60, Math.round(st.coin / n * 1.5));
    // 岁出：驻军 + 官俸
    let armies = 0;
    try { armies = (g.armies || []).filter(a => a.faction === me).length; } catch (e) {}
    const totalOut = Math.round(n * 60 + armies * 150);
    const surplus = totalIn - totalOut;
    const resGold = this._v28Num(res.money);

    const body = `
      <div class="v28-summary">
        <div class="v28-stat"><span>府库金</span><b style="color:var(--v28-coin)">💰 ${resGold}</b></div>
        <div class="v28-stat"><span>岁入</span><b style="color:var(--v28-safe)">+${totalIn}</b></div>
        <div class="v28-stat"><span>岁出</span><b style="color:var(--v28-danger)">-${totalOut}</b></div>
        <div class="v28-stat"><span>盈余</span><b style="color:${surplus >= 0 ? 'var(--v28-safe)' : 'var(--v28-danger)'}">${surplus >= 0 ? '+' : ''}${surplus}</b></div>
      </div>
      <div class="v28-summary">
        <div class="v28-stat"><span>通胀率</span><b style="color:${inflation > 30 ? 'var(--v28-danger)' : inflation > 15 ? 'var(--v28-warn)' : 'var(--v28-safe)'}">${inflation}%</b></div>
        <div class="v28-stat"><span>盐铁城数</span><b>${n}</b></div>
        <div class="v28-stat"><span>铸钱炉</span><b>🔨 ${st.coin}</b></div>
        <div class="v28-stat"><span>财政等级</span><b>Lv.${st.level}</b></div>
      </div>
      <div class="v28-status-line v28-wait">盐铁之利，入于县官；铸钱之权，操于国家。盐铁专卖则国用饶，铸钱无度则物价腾。</div>
      <div class="v28-section"><div class="v28-sec-title">◆ 盐铁专卖（每级岁入+200金）</div>
        <div class="v28-mon-list">
          <div class="v28-mon-row">
            <b>🧂 盐铁专卖</b>
            <span class="v28-mon-lv">Lv.${st.salt}</span>
            <button class="btn-small v28-mon-btn" onclick="__ui_._v28UpgradeMonopoly('salt')">榷盐<small>300金</small></button>
          </div>
          <div class="v28-mon-row">
            <b>⚒ 铁器专卖</b>
            <span class="v28-mon-lv">Lv.${st.iron}</span>
            <button class="btn-small v28-mon-btn" onclick="__ui_._v28UpgradeMonopoly('iron')">榷铁<small>300金</small></button>
          </div>
        </div>
      </div>
      <div class="v28-section"><div class="v28-sec-title">◆ 铸钱通货（每炉岁入+80金，但通胀+）</div>
        <div class="v28-coin-row">
          <span>开炉铸钱：🔨 ${st.coin} 炉</span>
          <button class="btn-small v28-coin-btn" onclick="__ui_._v28BuildCoin()">加铸一炉<small>500金</small></button>
        </div>
        <div class="v28-inflation-bar"><div class="v28-inflation-fill" style="width:${inflation}%"></div></div>
        <p class="v28-hint">通胀 ${inflation}%：铸钱逾多，物价愈贵；${inflation > 30 ? '已致恶性通胀，宜停铸。' : inflation > 15 ? '通胀渐起，宜节铸。' : '币值稳定。'}</p>
      </div>
      <div class="v28-section"><div class="v28-sec-title">◆ 均输平准（丰年籴、荒年粜）</div>
        <button class="btn-ancient v28-qs-btn" onclick="__ui_._v28Pingzhun()">⚖ 平准万物（耗粮 200，民心+）</button>
      </div>`;
    this._v28ModalShell('💰 盐铁通货 · 国计民生', body, 'v28-wide');
  }
  _v28UpgradeMonopoly(type) {
    const g = this.game; if (!g) return;
    const st = this._v28Store();
    const key = (type === 'iron') ? 'iron' : 'salt';
    if (st[key] >= 5) { this.toast('专卖已臻极品'); return; }
    const cost = 300;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v28Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    res.money -= cost;
    st[key] = this._v28Num(st[key]) + 1;
    this._v28Notify('专卖', `${key === 'salt' ? '盐铁' : '铁器'}专卖升至 Lv.${st[key]}，岁入+${200 * st[key]}。`, 'success');
    if (this.audio && this.audio.playCoin) { try { this.audio.playCoin(); } catch (e) {} }
    document.querySelectorAll('.v28-overlay').forEach(m => m.remove());
    this.showFinancePanelV28();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  _v28BuildCoin() {
    const g = this.game; if (!g) return;
    ...
    if (!res || this._v28Num(res.money) < cost) { this.toast(`金钱不足（需 ${cost} 金）`); return; }
    res.money -= cost;
    st.coin = this._v28Num(st.coin) + 1;
    this._v28Notify('铸钱', `新开铸钱炉一座，岁入+80金，通胀渐起。`, 'success');
    document.querySelectorAll('.v28-overlay').forEach(m => m.remove());
    this.showFinancePanelV28();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  _v28Pingzhun() {
    const g = this.game; if (!g) return;
    const res = (typeof g.getPlayerRes === 'function') ? g.getPlayerRes() : null;
    if (!res || this._v28Num(res.food) < 200) { this.toast('粮草不足（需 200 粮）'); return; }
    res.food -= 200;
    this._v28Notify('平准', '均输平准：丰年籴入、荒年粜出，物价平稳，民心稍附。', 'success');
    document.querySelectorAll('.v28-overlay').forEach(m => m.remove());
    this.showFinancePanelV28();
    if (typeof this.refreshUI === 'function') this.refreshUI();
  }
  ...
  showEspionagePanel() {
    ...
    const body = `
      <div class="v25-esp-upgrade" onclick="__ui_.showEspionagePanelV25()" title="V25.0 死间纵横：渗透 / 反间 / 离间 / 刺杀">🗡 V25 死间纵横 · 渗透反间离间刺杀 →</div>
