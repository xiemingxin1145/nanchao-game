...
export class AIPlayer {
  ...
  takeTurn(game) {
    ...
    const cacheKey = game.turn + ':' + myCities.length + ':' + totalCities;
    if (this._cacheKey !== cacheKey) {
      this._cacheKey = cacheKey;
      this._cacheWeak = (myCities.length / totalCities) < 0.15;
      this._cacheStrong = (myCities.length / totalCities) > 0.35;
    }
  ...
  researchTech(game, res) {
    ...
    // 候选：优先军事，其次经济，最后政治
    const order = ['military', 'economy', 'political'];
